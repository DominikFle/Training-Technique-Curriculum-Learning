...
# TODO
class DistillationTeacherDataset(Dataset):
    def __init__(
        self,
        model_extractor: IntermediateLayerExtractor,  # extractor to get intermediate layers from model
        stage="training",
        percent_of_dataset=1.0,
        labels=None,
    ):
    ...
    def __getitem__(self, index):
        ...
        label_classification = torch.tensor(self.labels[index])
        label_intermediate = self.model_extractor.get_intermediate_layers(feature)
        assert (
            label >= 0
        ...
        return (feature.float(), label_classification)


class DistillationTeacherDataModule(pl.LightningDataModule):
    def __init__(
        self,
        batch_size: int = 32,
